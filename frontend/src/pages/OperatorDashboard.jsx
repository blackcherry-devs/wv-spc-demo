  ...
  return (
    <div className="animate-in fade-in slide-in-from-right-4 duration-500">
      <OperationsGrid 
        ...
        onSelectOperation={(op) => navigate(`/operator/proceso/${processId}/operacion/${encodeURIComponent(op)}`)} 
      />
    </div>
  );
};
    ...
    <div className="animate-in fade-in slide-in-from-right-4 duration-500">
      <header className="mb-8">
        ...
        <div className="flex items-center justify-between">
          ...
          <button 
            onClick={() => navigate(`/operator/proceso/${processId}/operacion/${operationId}`)}
...
const ProcessCardsWrapper = () => {
  ...
  return (
    <div className="animate-in fade-in duration-500">
      ...
      <ProcessCards onSelectProcess={(p) => navigate(`/operator/proceso/${encodeURIComponent(p)}`)} />
    </div>
  );
};
...
const OperatorDashboard = () => {
  ...
  // Strip the /operator prefix for path parsing
  const subPath = location.pathname.replace(/^\/operator/, '') || '/';
    ...
    <div className="bg-background text-on-surface h-[calc(100vh-4rem)] flex font-sans w-full">
      ...
      <aside ref={sidebarRef} className="fixed left-0 top-16 h-[calc(100vh-4rem)] w-64 z-40 bg-surface border-r border-outline/20 shadow-lg flex flex-col py-lg pt-8">
        ...
        <div className="px-6 mb-lg">
          <h2 className="font-title-md text-title-md text-primary nav-text whitespace-nowrap">Panel Operador</h2>
          <p className="font-label-md text-label-md text-on-surface-variant nav-text whitespace-nowrap">Captura de Parámetros</p>
        </div>
        <nav className="flex-1 space-y-sm mt-4 px-2">
          ...
          <div>
            <div 
              onClick={() => activeProcess && navigate(`/operator/proceso/${encodeURIComponent(activeProcess)}`)}
      ...
      {/* Footer */}
      <footer className={`fixed bottom-0 right-0 bg-surface-container-lowest border-t border-outline/10 flex justify-between items-center px-8 py-2 z-50 transition-all duration-500 ease-out ${isMinimized ? 'left-20' : 'left-64'}`}>
