// Volkswagen SCP Demo - Mock Data Store for Offline/Serverless Deployment
// This file keeps all operations data identical to the backend server.
...
export const mockLavadoraCentralData = {
  laboratorio: [
    {
      metricId: 'conc-ph',
...
// Returns a deep copy of the mock data with a simulated network latency of 500ms
export const fetchMockOperationData = (operationId) => {
  return new Promise((resolve, reject) => {
    setTimeout(() => {
      const lowerOp = (operationId || '').toLowerCase();
      if (lowerOp === 'lavadora central' || lowerOp === 'lavadoracentral') {
        // Deep copy to prevent state mutation leaks
        resolve(JSON.parse(JSON.stringify(mockLavadoraCentralData)));
      } else {
        resolve({
          laboratorio: [],
          proceso: [],
